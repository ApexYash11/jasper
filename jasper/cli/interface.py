...
def _render_box_table(headers: list[str], rows: list[list[str]]) -> str:
    ...
    top = (
        _BOX_CHARS["top_left"]
        + _BOX_CHARS["top_t"].join(
            _BOX_CHARS["horizontal"] * (width + 2) for width in col_widths
        )
    ...
    header_row = (
        ...
        + _BOX_CHARS["vertical"].join(
            f" {pad_cell(header, col_widths[idx], False)} "
    ...
    for row in rows:
        ...
        data_rows.append(
            _BOX_CHARS["vertical"]
            + _BOX_CHARS["vertical"].join(
                f" {pad_cell(padded[idx], col_widths[idx], align_right[idx])} "
...
def _format_cli_markdown(text: str) -> str:
    ...
    return _transform_markdown_tables_to_box(
        _compact_cli_layout(_fix_markdown_tables(text))
    )
...
def render_banner():
    ...
    subtitle = Text(
        ...
        style=f"bold #000000 on {THEME['Accent']}",
    )
...
def build_persistent_board():
    ...
    """
    root_tree = Tree(
        f"[bold {THEME['Brand']}] MISSION CONTROL[/bold {THEME['Brand']}]",
        guide_style="",
    )

    # Create three phase sections (persistent tree nodes)
    planning_node = root_tree.add(
        f"[bold {THEME['Accent']}]▸ PLANNING[/bold {THEME['Accent']}]"
    )
    execution_node = root_tree.add(
        f"[bold {THEME['Accent']}]▸ EXECUTION[/bold {THEME['Accent']}]"
    )
    synthesis_node = root_tree.add(
        f"[bold {THEME['Accent']}]▸ SYNTHESIS[/bold {THEME['Accent']}]"
    )
...
def render_forensic_report(report: FinalReport):
    ...
    dash_table.add_row(
        "VALIDATION",
        Text("PASSED" if report.is_valid else "FAILED", style=status_style),
    )
    ...
    dash_table.add_row(
        "CONFIDENCE",
        Text(
            f"{report.confidence_score:.0%} ({report.confidence_score:.2f})",
            style=conf_style,
        ),
    )
    ...
    evidence_table = Table(
        title="[bold]1. EVIDENCE MATRIX[/bold]", box=box.ROUNDED, expand=True
    )
    ...
    for item in report.evidence_log:
        evidence_table.add_row(
            item.id, item.metric, str(item.value), item.source, item.status
        )
    ...
    # 4. Audit Trail (Mini)
    audit_table = Table(
        title="[bold]3. EXECUTION AUDIT TRAIL[/bold]", box=box.SIMPLE, expand=True
    )
    ...
    if report.audit_trail:
        ...
        audit_table.add_row(
            "[dim]No financial data tasks[/dim]",
            "[dim]N/A[/dim]",
            "[dim]Qualitative analysis[/dim]",
        )

    return Group(
        Panel(
            dash_table,
            title="[bold]FORENSIC METADATA DASHBOARD[/bold]",
            border_style=THEME["Brand"],
        ),
        evidence_table,
        synthesis_panel,
        audit_table,
        Text(
            f"Jasper v{report.version} | Deterministic Forensic Artifact",
            justify="center",
            style="dim",
        ),
    )
