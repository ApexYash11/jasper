...
class JasperController:
    ...
    async def run(self, query: str) -> Jasperstate:
        ...
        try:
            ...
            self.logger.log(
                "PLAN_CREATED",
                {
                    "plan": [t.dict() for t in state.plan],
            ...
            if not state.validation.is_valid:
                self.logger.log(
                    "VALIDATION_FAILED", {"issues": state.validation.issues}
                )
                state.status = "Failed"
                return state
            ...
            try:
                ...
                state.final_answer = await self.synthesizer.synthesize(
                    state, token_callback=token_cb
                )
                ...
                self.logger.log(
                    "REPORT_CREATED", {"report_valid": state.report.is_valid}
                )
                ...
                if "524" in error_msg or "provider returned error" in error_msg.lower():
                    ...
                    state.error = (
                        "LLM authentication failed. Check your OpenRouter API key."
                    )
                ...
                self.logger.log(
                    "SYNTHESIS_ERROR",
                    {"error": state.error, "source": state.error_source},
                )
                state.status = "Failed"
            return state
    ...
    def _build_final_report(self, state: Jasperstate) -> FinalReport:
        ...
        logic_constraints = (
            state.report.logic_constraints
            if state.report and hasattr(state.report, "logic_constraints")
            else {}
        )
        if not state.plan:
            logic_constraints["QUERY_INTENT"] = (
                "Qualitative research selected; financial statement fetching skipped."
            )

        # Simple Inference Link (Linking all evidence to the synthesis)
        inference_map = []
        if evidence_log:
            inference_map.append(
                InferenceLink(
        ...
        report = FinalReport(
            ...
            confidence_breakdown=state.validation.breakdown
            if state.validation
            else None,
