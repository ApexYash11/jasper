...
class Task(BaseModel):
    ...
    tool_name: Optional[str] = Field(
        default=None, description="Name of the tool to be used for the task"
    )
...
class InferenceLink(BaseModel):
    ...
    confidence: float = Field(
        default=1.0, description="Confidence in this specific inference"
    )
...
class ValidationResult(BaseModel):
    ...
    confidence: float = Field(
        default=0.0, description="Confidence score of the validation result"
    )
...
class FinalReport(BaseModel):
    ...
    report_mode: ReportMode = Field(
        default=ReportMode.GENERAL,
        description="The inferred analytical mode of the report",
    )
    ...
    version: str = Field(
        default=__version__, description="Jasper version at report generation time"
    )

    # Data sourcing
    data_sources: List[str] = Field(
        default_factory=list,
        description="List of data providers used (e.g., ['yfinance', 'Alpha Vantage'])",
    )
    tickers: List[str] = Field(
        default_factory=list, description="Financial instruments analyzed"
    )

    # Forensic Evidence & Analysis
    evidence_log: List[EvidenceItem] = Field(
        default_factory=list, description="Structured log of all evidence artifacts"
    )
    inference_map: List[InferenceLink] = Field(
        default_factory=list, description="Map of claims to their supporting evidence"
    )
    logic_constraints: Dict[str, str] = Field(
        default_factory=dict,
        description="Constraints applied during synthesis (e.g. data gaps)",
    )
    audit_trail: List[TaskExecutionDetail] = Field(
        default_factory=list, description="Forensic audit of execution steps"
    )

    # Results (Legacy/Secondary)
    synthesis_text: str = Field(
        ..., description="Final synthesized analysis (markdown or plain text)"
    )

    # Validation & confidence
    is_valid: bool = Field(..., description="Whether validation passed")
    validation_issues: List[str] = Field(
        default_factory=list, description="Validation issues encountered"
    )
    confidence_score: float = Field(
        default=0.0, description="Overall confidence (0.0-1.0)"
    )
    ...
    task_results: Dict[str, Any] = Field(
        default_factory=dict, description="Raw task execution results"
    )
...
class Jasperstate(BaseModel):
    ...
    current_task_index: int = Field(
        default=0, description="Index of the current task being executed"
    )

    task_results: Dict[str, Any] = Field(
        default_factory=dict, description="Results of executed tasks, keyed by task ID"
    )

    validation: Optional[ValidationResult] = Field(
        default=None, description="Validation result of the current state"
    )

    retries: int = Field(
        default=0, description="Number of retries attempted for failed tasks"
    )
    max_retries: int = Field(
        default=3, description="Maximum number of retries allowed for failed tasks"
    )
    ...
    status: Literal[
        ...
        "Completed",
        "Failed",
    ] = Field(default="Planning", description="Current status of the agent")

    final_answer: Optional[str] = Field(
        default=None, description="The final answer generated by Jasper"
    )
    error: Optional[str] = Field(
        default=None, description="Error message if the run failed unexpectedly"
    )
    error_source: Optional[str] = Field(
        default=None,
        description="Source of the error: 'data_provider', 'llm_service', 'llm_auth', 'llm_timeout', 'llm_unknown', or 'query'",
    )

    # Report object (populated after synthesis)
    report: Optional[FinalReport] = Field(
        default=None, description="Audit-ready report for export"
    )
