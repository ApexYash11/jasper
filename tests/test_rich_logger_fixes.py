...
class TestDebounceInterval:
    """Verify debounce interval is set to 80ms for smoother streaming."""

    def test_min_update_interval_is_80ms(self):
        """Debounce interval should be 80ms for smoother Live updates."""
        ...
        assert logger._min_update_interval == 0.08, (
            f"Expected _min_update_interval=0.08 (80ms), got {logger._min_update_interval}"
        )
        print("   ✅ Debounce interval: 80ms")
    ...
    def test_debounce_logic_respects_interval(self):
        ...
        # Immediate second call should return False (within 80ms)
        assert logger._should_update_live() is False, "Second call should be debounced"

        # After 80ms, should allow update
        time.sleep(0.09)
        assert logger._should_update_live() is True, "Call after 80ms should be allowed"
...
class TestStreamingPreviewThrottle:
    """Verify synthesis preview throttle is 60 chars for faster preview updates."""

    def test_preview_update_threshold_is_60(self):
        """Preview should update every 60 accumulated characters."""
        from jasper.cli.main import RichLogger
        ...
        assert logger._preview_update_every_chars == 60, (
            f"Expected _preview_update_every_chars=60, got {logger._preview_update_every_chars}"
        )
        print("   ✅ Synthesis preview throttle: 60 chars")
    ...
    def test_synthesis_buffer_accumulation(self):
        ...
        logger.synthesis_buffer = "A" * 40  # Below threshold
        ...
        assert not should_update, "Should NOT update at 40 chars (threshold is 60)"

        # Continue to 80 chars
        logger.synthesis_buffer = "A" * 80  # Above threshold
        char_delta = len(logger.synthesis_buffer) - logger._last_stream_update_chars

        should_update = char_delta >= logger._preview_update_every_chars
        assert should_update, "Should update at 80 chars (exceeds 60 threshold)"

        print("   ✅ Synthesis buffer accumulation respects 60-char threshold")
...
class TestRuleSeparatorsRemoved:
    ...
    def test_render_final_report_no_rule_separators(self):
        ...
        assert not has_rule, "Final report content should not contain Rule separators"
...
class TestLoggerIntegration:
    ...
    def test_logger_event_flow_with_debounce(self):
        ...
        assert logger._should_update_live() is False, (
    ...
    def test_configuration_matches_expected_values(self):
        ...
        config_checks = {
            "_min_update_interval": (logger._min_update_interval, 0.08),
            "_preview_update_every_chars": (logger._preview_update_every_chars, 60),
        }
...
class TestOutputClarity:
    ...
    def test_no_repeated_horizontal_lines_in_config(self):
        ...
        # At 0.08s debounce, Live widget (~12 refreshes/sec) will group updates
        # preventing excessive re-rendering of tree guide lines
        assert logger._min_update_interval == 0.08, (
            "Debounce must be ≥80ms to prevent line artifacts"
        )
